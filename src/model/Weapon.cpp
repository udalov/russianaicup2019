...
string Weapon::toString() const {
    string ans = weaponTypeToString(type) + " " + to_string(magazine) + " " + ::toString(spread);
    if (fireTimer.has_value()) ans += " " + ::toString(*fireTimer);
    if (lastAngle.has_value()) ans += " " + ::toString(*lastAngle);
